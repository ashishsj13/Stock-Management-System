...
const Home = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const res = await axios.post("http://localhost:5000/createproduct", inputUser);
    console.log(res);
    fetchAllUser();
  };
        ...
        <table className="text-center table-center w-full">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 bg-gray-800 text-white">Total Price</th> {/* Added Total Price column */}
          ...
          <tbody>
            {productData.map((item, i) => (
              <tr key={item._id}> {/* Added key prop */}
                ...
                <td className="border px-4 py-2">{item?.quantity * item?.price}</td> {/* Calculating Total Price */}
